...
source("src_R/km.R")
source("src_R/ekm.R")
source("src_R/eiasc.R")
source("src_R/da.R")
source("src_R/dastar.R")
source("src_R/dand.R")
source("src_R/costrwsr.R")
source("src_R/ecostrwsr.R")
source("src_R/sc.R")
...
for(i in 1:length(N)) {
    ...
    a.lower <- runif(m, 1, 2)
    a.upper <- a.lower * runif(m, 1, 2)
    b <- runif(m, 1, 2)
    c <- runif(m, 0, 10)

    x <- replicate(m, sort(runif(N[i], 0, 10)))
    sorted <- T

    wl <- wr <- matrix(,N[i],m)
    for(j in 1:m) {
        wl[,j] <- 1 / ( 1 + (((x[,j] - c[j])/a.lower[j])^2)^b[j])
        wr[,j] <- 1 / ( 1 + (((x[,j] - c[j])/a.upper[j])^2)^b[j])
    }
...
library(R.matlab)
writeMat("../../results/RExample1.mat", times=times)
